    ...
    return (
        <div>
            ...
            <div className="container">
                <div className="row">
                    <section className="discussions" style={{
                        overflowY: "scroll",
                    }}>
                        ...
                        {filterItems?.map((item, index) =>
                            ...
                            <div className="discussion" key={index} onClick={(e) => setCurrentUser(item)}>
                                <div className="photo" style={{ backgroundImage: item.profilePic ? `url(${item.profilePic})` : "url(https://card.thomasdaubenton.com/img/photo.jpg)" }}>
                                    {activeUser && activeUser.length > 0 && activeUser.some(u => u.userId === item._id) ? <div className="online"></div> : ""}
                                </div>
                                <div className="desc-contact">
                                    <p className="name">{item?.firstName}</p>
                                    <p className="message">You can't see me</p>
                                </div>
                                <div className="timer">{moment(item?.createdAt).startOf('mini').fromNow()}</div>
                            </div>
                        )}
                    </section>
                    <section className="chat">
                        <div className="header-chat">
                            ...
                            >
                                <div>
                                    ...
                                    <p className="name"> {currentUser ? `Chat With ${currentUser.firstName}` : "please select your friend"}</p>
                            ...
                            {message && message.length > 0 ? message.map((msg, index) =>
                                ...
                                <div key={index}>
...

                                    {
                                        ...
                                        msg.senderId === myInfo.data._id ? <div className="message text-only">
                                            ...
                                            <div>
                                                ...
                                                >
                                                    ...
                                                    <p className="text"> {msg?.message?.text}
                        ...
                        >
                            <div>
                                {typingMessage && typingMessage.message && typingMessage.senderId === currentUser._id ? "typing..." : ""}
                            </div>
                            <input type="text" onChange={(e) => messageChangeHandler(e)} value={newMessage} className="write-message" placeholder="Type your message here"></input>
                            <button onClick={sendMessage} className="icon send fa fa-paper-plane-o clickable" aria-hidden="true">send</button>
                        </div>
                    </section>
                </div>
            </div >
